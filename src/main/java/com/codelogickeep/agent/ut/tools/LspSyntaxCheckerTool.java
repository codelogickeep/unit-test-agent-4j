...
public class LspSyntaxCheckerTool implements AgentTool {
    ...
    // 用于等待特定文件诊断更新的 Latch Map
    private final Map<String, CountDownLatch> fileLatches = new ConcurrentHashMap<>();
    // 诊断接收时间戳，用于判断诊断是否稳定
    ...
    /**
     * Initialize LSP server for the project. Called automatically when use-lsp is
     * enabled.
    ...
    public String checkSyntaxWithLsp(
        ...
        try {
            ...
            fileLatches.put(uri, latch);

            // 清除旧的诊断信息，确保获取到的是新的
            diagnosticsCache.remove(uri);

            // 2. 打开文件 (如果是已打开的文件，JDT LS 可能会忽略或更新，这里假设是无状态检查，最好配对 close)
            ...
            // 3. 等待诊断结果（LSP 会异步推送）
            log.info("Waiting for diagnostics for: {}", filePath);
            boolean received = latch.await(15, TimeUnit.SECONDS);

            if (!received) {
                log.warn("Timeout waiting for diagnostics for {}", filePath);
            }
    ...
    private class LspClient implements LanguageClient {
        ...
        public void publishDiagnostics(PublishDiagnosticsParams diagnostics) {
            ...
            // 通知等待该文件的线程
            CountDownLatch latch = LspSyntaxCheckerTool.this.fileLatches.get(uri);
            if (latch != null) {
                latch.countDown();
            }
        }
